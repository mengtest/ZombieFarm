 ...
 */
-(void)InitEngine:(NSString*)sdkAppID openID:(NSString*)openID;
...
-(void)SetLogLevel:(ITMG_LOG_LEVEL)logLevel enableWrite:(bool)enableWrite enablePrint:(bool)enablePrint;
 ...
 *
 */
-(void)EnterRoom:(int)roomId roomType:(int)roomType authBuffer:(NSData*)authBuffer;
...
/**
 *  停止音频引擎，app需要播放音乐时要先停掉我们的sdk。播放完了之后再调用resumeAudio重启音频引擎
 */
-(QAVResult)PauseAudio;
/**
 *  恢复音频引擎
 */
-(QAVResult)ResumeAudio;
...
// 只是快捷方式。EnalbeMic(value) = EnableAudioCaptureDevice(value) + EnableAudioSend(value)
...
-(void)SetMaxMessageLength:(int)msTime;
