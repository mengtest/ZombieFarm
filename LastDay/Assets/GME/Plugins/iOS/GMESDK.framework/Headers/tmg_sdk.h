...
typedef enum {
	...
	//Notification of the room disconnection due to network or other reasons, which triggers automatically.
	ITMG_MAIN_EVENT_TYPE_ROOM_DISCONNECT = 3,
	//Notification of the updates of room members, which triggers automatically, the notification contains detailed information, refer to ITMG_EVENT_ID_USER_UPDATE.
...
typedef enum
{
	ITMG_GAME_AUDIO_MODE_WORLD = 0x0,
	ITMG_GAME_AUDIO_MODE_TEAM = 0x1,
}ITMG_GAME_AUDIO_MODE;
...
public:
    ...
    //Set the Log level
	//		[in]logLevel --> Print the level of Log
	//		[in]enableWrite -> write log into logfile default TRUE 
	//		[in]enablePrint -> print log to console default TRUE 
	virtual void SetLogLevel(ITMG_LOG_LEVEL logLevel, bool enableWrite, bool enablePrint) = 0;

	// Set the folder path of the Log
	// [in] logDir	-->	Folder path
	virtual void SetLogPath(const char* logDir) = 0;
	...
	virtual void Init(const char* sdkAppId, const char* openId) = 0;
	...
	virtual void EnterRoom(int roomId, ITMG_ROOM_TYPE roomType, const char* authBuff, int buffLen) = 0;

	// An advance API of EnterRoom, more information on Official Website
	//		[in]roomId			-->The room u are in, it can't be 0.
	//		[in]roomType		--> Now only ITMG_ROOM_TYPE_FLUENCY can be used, Ohter type are reserved.
	//		[in]authBuff		-->	Authentication code in Tencent Cloud, 
	//		[in]buffLen			-->	String length of Authentication cod
	//		[in]teamId			-->	The team u are in, it can't be 0.
	//		[in]gameAudioMode	-->	Only take effect when teamId is not equal to 0.
	//							--> 0 means WordMode and 1 means TeamMode.
	//							--> can change any time after enter the room,
	//							--> relatived api : ChangeGameAudioMode
	virtual void EnterTeamRoom(int roomId, ITMG_ROOM_TYPE roomType, const char* authBuff, int buffLen, int teamId, ITMG_GAME_AUDIO_MODE gameAudioMode) = 0;
...
public:
	...
	// see EnterTeamRoom
	virtual void ChangeGameAudioMode(ITMG_GAME_AUDIO_MODE gameAudioMode) = 0;
	...
	virtual void UpdateAudioRecvRange(int range) = 0;
	...
	virtual void UpdateSelfPosition(int position[3], float axisForward[3], float axisRightward[3], float axisUpward[3]) = 0;
};
...
public:
	...
	// a shortcut of accessing microphone. EnableMic(value) = EnableAudioCaptureDevice(value) + EnableAudioSend(value)
	...
	// a shortcut of accessing speaker. EnableSpeaker(value) = EnableAudioPlayDevice(value) + EnableAudioRecv(value)
	...
	//		[in]volume--->Audio Volume reference range[0 to 100],default value is 100
	...
	//		[in]nVolume	-->	Audio Volume reference range[0 to 100],default value is 100
...
public:
	...
	virtual void ApplyPTTAuthbuffer(const char* authBuffer, int authBufferLen) = 0;
