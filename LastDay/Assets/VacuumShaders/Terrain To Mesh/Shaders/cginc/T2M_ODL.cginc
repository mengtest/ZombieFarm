...
#include "../../../../Shaders/ME/ME_SimulatePointLight.cginc"
...
fixed4 frag (vOutput IN) : SV_Target
{
    ...
    #ifdef LIGHTMAP_OFF
		#ifdef V_T2M_BUMP
			fixed diff = max (0, dot (normal, IN.lightDir) * 0.5 + 0.5);
		#else
			fixed diff = max (0, dot (normal, _WorldSpaceLightPos0.xyz) * 0.5 + 0.5);
		#endif
    ...
    c.rgb = CircleLighten(IN.worldPos, c.rgb, 1.72);
