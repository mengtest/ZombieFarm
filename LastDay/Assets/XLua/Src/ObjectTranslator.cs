...
CONNECT_BASE_TYPE:
            ...
            // 枚举类在使用时自动Wrap，无需关联
            if (!type.IsEnum) {
                if (type.IsValueType) UnityEngine.Debug.LogWarningFormat("未导出的值类型({0})被压栈", type);

                for (var baseType = type.BaseType; ; baseType = baseType.BaseType) {
                    if (baseType != null){
                        if (delayWrap.TryGetValue(baseType, out loader)) {
                            delayWrap.Remove(baseType);
                            loaded_types.Add(baseType, true);
                            loader(L);
                        } else if (!loaded_types.ContainsKey(baseType)) continue;

                        // 拷贝基类的对象元表到当前类的对象元表
                        L.L_GetMetaTable(baseType.FullName);
                        var basePos = L.GetTop();
                        L.L_GetMetaTable(type.FullName);
                        var pos = L.GetTop();
                        if (L.IsTable(basePos)) {
                            L.PushNil();
                            while (L.Next(basePos)) {
                                L.PushValue(-2);
                                L.Insert(-2);
                                L.RawSet(pos);
                            }
