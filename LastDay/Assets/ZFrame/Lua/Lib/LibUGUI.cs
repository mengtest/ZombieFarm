...
public static class LibUGUI
{
    ...
    {
        None, Horizontal, Vertical, HorizontalAndVertical,
    }
    ...
    private static int CreateWindow(ILuaState lua)
    {
        ...
        if (luaT == LuaTypes.LUA_TSTRING) {
            ...
            var prefab = lua.ToUserData(1) as GameObject;
            if (prefab) {
                GameObject go = uimgr.CreateWindow(prefab, index, canvasId);
                lua.PushLightUserData(go);
                return 1;
    ...
    private static int SetLocalize(ILuaState lua)
    {
        ...
        Localize.DEF_LANG = lua.ToString(2);
...
//        return 1;
        ...
        if (path != null) {
            ...
            if (sp) {
                ...
                if (!sp.overrideSprite) {
                    AssetsMgr.A.LoadAsync(typeof(SpriteAtlas),
                        string.Format("Atlas/{0}/{0}", sp.atlasName),
                        LoadMethod.Default, OnSpriteAtlasLoaded, sp);
                }
            } else if (img) {
                img.overrideSprite = UISprite.LoadSprite(path, false);
    ...
    private static int DOAnchor(ILuaState lua)
    {
        ...
        if (from && from.gameObject.activeInHierarchy && to && to.gameObject.activeInHierarchy) {
            ...
            if (autoAnchor) {
                ...
                v2Ret = cv.worldCamera.AnchorPosition(from, pivotF, to, pivotT, offset);
                if (set) {
    ...
    {
        var tgr = lua.ToComponent(1, typeof(UIEventTrigger)) as UIEventTrigger;
        if (tgr) {
            var type = (TriggerType)lua.ToEnumValue(2, typeof(TriggerType));
            var name = (UIEvent)lua.ToEnumValue(3, typeof(UIEvent));
            var param = lua.ToString(4);
            tgr.SetEvent(type, name, param);
            if (type == TriggerType.Longpress) tgr.gameObject.NeedComponent(typeof(UILongpress));
        }
        return 0;
    }
    #endregion
    ...
    private static int DOFade(ILuaState lua)
    {
        ...
        if (go && go.activeInHierarchy) {
            ZTween.Stop(go);
            var tw = group != null ?
                FadeTool.DOFade(go, (FadeGroup)group, reset, foward) :
                FadeTool.DOFade(go, reset, foward);
            if (func != null) {
                if (tw != null) {
                    tw.CompleteWith(_ => { func.Action(go); func.Dispose(); });
                } else {
                    MEC.Timing.RunCoroutine(LibUnity.LuaInvoke(LuaScriptMgr.Instance, func, 0, go));
                }
            }
        }
        return 0;
    ...
    private static ITweenable ToTweenable(object tweenObject, object tweenType)
    {
        ...
        if (go && go.activeInHierarchy) {
            ...
            if (tweenName != null) {
                ...
                if (tweenable == null) {
                    switch (tweenName) {
                        case "Position": tweenable = go.NeedComponent(typeof(TweenPosition)) as ITweenable; break;
                        case "PositionW": tweenable = go.NeedComponent(typeof(TweenPositionW)) as ITweenable; break;
                        case "Rotation": tweenable = go.NeedComponent(typeof(TweenRotation)) as ITweenable; break;
                        case "EulerAngles": tweenable = go.NeedComponent(typeof(TweenEulerAngles)) as ITweenable; break;
                        case "Scale": tweenable = go.NeedComponent(typeof(TweenScaling)) as ITweenable; break;
                        case "Transform": tweenable = go.NeedComponent(typeof(TweenTransform)) as ITweenable; break;
                        case "Alpha": tweenable = go.NeedComponent(typeof(TweenAlpha)) as ITweenable; break;
                        case "Size": tweenable = go.NeedComponent(typeof(TweenSize)) as ITweenable; break;
                        default: break;
                    }
                }
    ...
    private static int Overlay(ILuaState lua)
    {
        ...
        if (trans) {
            if (lua.IsClass(2, UnityEngine_Vector3.CLASS)) {
                ...
                trans.Overlay(tarTrans.position, tarCam);
            }
        }
        lua.PushBoolean(true);
        return 1;
    ...
    private static int Indicate(ILuaState lua)
    {
        ...
        if (trans) {
            ...
            if (target) {
                var indicator = trans.gameObject.NeedComponent<CurvedWorldUIIndicator>();
