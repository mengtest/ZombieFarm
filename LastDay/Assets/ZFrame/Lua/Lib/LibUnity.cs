...
public static class LibUnity
{
    ...
    private static int PlayAudio(ILuaState lua)
    {
        ...
        if (!string.IsNullOrEmpty(eventName)) {
            ...
            var emitter = FMODUnity.FMODMgr.Play(eventName, parent, fadeout);
    ...
    public static IEnumerator<float> LuaInvoke(MonoBehaviour mono, LuaFunction func, float delay, object param)
    {
        if (delay > 0) {
    ...
    private static IEnumerator<float> LuaInvokeRepeating(MonoBehaviour mono, LuaFunction func, float delay, float wait, object param)
    {
        if (delay > 0) {
            yield return MEC.Timing.WaitForSeconds(delay);
        } else {
        ...
        using (func) {
            for (int i = 0; mono && mono.isActiveAndEnabled; ++i) {
                ...
                if (wait > 0) {
                    yield return MEC.Timing.WaitForSeconds(wait);
                } else {
                    yield return MEC.Timing.WaitForOneFrame;
                }
            }
    ...
    private static int Invoke(ILuaState lua)
    {
        ...
        if (mono.isActiveAndEnabled) {
            ...
            if (func != null) {
                ...
                MEC.Timing.RunCoroutine(LuaInvoke(mono, func, wait, param), 
                    tag ?? mono.GetHashCode().ToString());
            } else {
    ...
    private static int InvokeRepeating(ILuaState lua)
    {
        ...
        if (mono.isActiveAndEnabled) {
            ...
            if (func != null) {
                ...
                MEC.Timing.RunCoroutine(LuaInvokeRepeating(mono, func, delay, wait, param),
                    tag ?? mono.GetHashCode().ToString());
            } else {
    ...
    private static int CancelInvoke(ILuaState lua)
    {
        if (lua.Type(1) == LuaTypes.LUA_TSTRING) {
            ...
            var mono = lua.ToAsyncMono(1);
            if (mono && mono.isActiveAndEnabled) {
                MEC.Timing.KillCoroutines(mono.GetHashCode().ToString());
            }
        }
    ...
    private static int UnloadLevel(ILuaState lua)
    {
        ...
        AssetsMgr.A.Loader.ClearPreload();
        return 0;
    }
