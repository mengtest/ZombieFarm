        ...
        public struct Settings
        {
            ...
            public void OnPreprocess(TextureImporter ti)
            {
                if (ContainsAsset(folders, ti.assetPath)) {
                    if (ContainsFlag(flags, (int)Prop.MaxTextureSize)) {
                        ti.maxTextureSize = MaxTextureSize;
                    } else if (ContainsFlag(flags, (int)Prop.CompressionQuality)) {
                        ti.compressionQuality = CompressionQuality;
                    } else if (ContainsFlag(flags, (int)Prop.CrunchedCompression)) {
                        ti.crunchedCompression = CrunchedCompression;
                    } else if (ContainsFlag(flags, (int)Prop.TextureCompression)) {
                        ti.textureCompression = TextureCompression;
                    } else if (ContainsFlag(flags, (int)Prop.MipmapEnabled)) {
