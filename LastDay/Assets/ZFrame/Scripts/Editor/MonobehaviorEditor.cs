...
public class MonoBehaviorEditor : Editor
{
    ...
    protected void ShowList(string desc, ICollection collection)
	{
        EditorGUILayout.LabelField(desc, string.Format("数量={0}", collection.Count));
    ...
    protected void ShowObject(string desc, object obj)
    {
        if (obj is IDictionary) {
            ShowDictionary(desc, obj as IDictionary);
        } else if (obj is ICollection) {
            ShowList(desc, obj as ICollection);
        } else  {
