...
namespace ZFrame
{
    ...
    {
        private const string fpsLabel = "<color=red>{2:N0}Ping</color> {0:N0}FPS {1:N0}MS</color>";
        ...
        private void SetPingValue(float ping)
        {
            m_fPing = ping;
        }
        ...
        private void OnGUI()
        {
            GUI.Label(new Rect(Screen.width - 200, 0, 200, 30), string.Format(m_HtmlColorTag + fpsLabel, fps, ms, m_fPing), m_Style);
        }
    }
}
