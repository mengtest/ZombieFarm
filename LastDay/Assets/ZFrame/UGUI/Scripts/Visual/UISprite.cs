...
namespace ZFrame.UGUI
{
    ...
    public class UISprite : Image, ITweenable, ITweenable<float>, ITweenable<Color>
    {
        ...
        [NoToLua]
        public static SpriteAtlas LoadAtlas(string atlasName, bool warnIfMissing)
        {
            ...
            if (AssetsMgr.A == null) {
                ...
                var obj = AssetLoader.EditorLoadAsset(null, atlasPath);
                var atlas = obj as UIAtlas;
                ret = atlas ? atlas.atlas : obj as SpriteAtlas;
#endif
            } else {
                ...
                if (AssetsMgr.A.Loader.TryGetAssetBundle((atlasRoot + atlasName).ToLower(), out abRef)) {
                    var atlas = abRef.Load(atlasName, typeof(UIAtlas)) as UIAtlas;
                    ret = atlas ? atlas.atlas : abRef.Load(atlasName, typeof(SpriteAtlas)) as SpriteAtlas;
                    if (ret != null) LoadedAtlas.Add(atlasName, ret);
                }
            }

            if (warnIfMissing && ret == null) LogMgr.W("Load Atlas[{0}] fail!", atlasName);

            return ret;
        }

        [NoToLua]
        public static Sprite LoadSprite(string path, bool warnIfMissing)
        {
            var atlasName = SystemTools.GetDirPath(path);
            var spriteName = System.IO.Path.GetFileName(path);
            var atlas = LoadAtlas(atlasName, warnIfMissing);
        ...
        public string spriteName {
            get { return overrideSprite ? overrideSprite.name : m_SpriteName; }
            set {
                m_SpriteName = value;
        ...
        private void OnSpriteLoaded(string a, object o, object param)
        {
            ...
            if (!overrideSprite) {
                LogMgr.W("Load <Sprite> Fail! path = \"{0}\"", a);
            }
        }
        ...
        public void SetSprite(string path, bool warnIfMissing)
        {
            ...
            if (string.IsNullOrEmpty(m_AtlasName)) {
                ...
                m_Atlas = LoadAtlas(m_AtlasName, warnIfMissing);
                if (m_Atlas) {
        ...
        protected void AutoLoadSprite()
        {
            if ((overrideSprite == null || overrideSprite.texture == null) && m_Atlas && !string.IsNullOrEmpty(m_SpriteName)) {
                ...
                if (overrideSprite == null) {
                    LogMgr.W("Load <Sprite:{0}/{1}> Fail! @ {2}", m_AtlasName, m_SpriteName, this.GetHierarchy());
#if UNITY_EDITOR
        ...
        protected void InitAtlasSprite()
        {
            if (m_Atlas == null && !string.IsNullOrEmpty(m_AtlasName)) {
                m_Atlas = LoadAtlas(m_AtlasName, true);
                if (m_Atlas == null) {
#if UNITY_EDITOR
                    if (Application.isPlaying)
#endif
        ...
        public void Load(string path, DelegateObjectLoaded onLoaded = null, object param = null)
        {
            if (string.IsNullOrEmpty(path) || path.EndsWith('/')) {
                ...
                AssetsMgr.A.LoadAsync(typeof(Sprite), path, LoadMethod.Cache, loadedCfg, param);
            }
        }
