...
local function callhook(k, ...)
	local f = Hooks[k]
	if f then return trycall(f, ...) end
end
...
RefDEF.__index = function (t, k)
	...
	if preffix == "lb" then
		val = libunity.FindByName(go, k, "UIText")
		if val == nil then
	...
	elseif preffix == "sp" then
		...
		elseif preffix == "inp" then
			val = go:GetComponent("TMP_InputField") or go:GetComponent("InputField")
...
local function u_create(prefab, depth, instantly)
...
function OBJDEF:open(instantly)
	...
	self.go = u_create(self.path, self.depth, instantly)
	...
	if self.stacked then
		...
		if TopWnd then
			while TopWnd do
				...
				if not TopWnd:is_opened() then TopWnd:open() end
...
function ui.show(path, depth, Context, instantly)
	...
	local Wnd = LCWnds[lcName] or OBJDEF.new(path, depth)
	Wnd.Context = Context
	Wnd:open(instantly)
	return Wnd
end
...
function ui.setvisible(name, visible)
	local Wnd = ui.find(name)
	if Wnd then libugui.SetVisible(Wnd.go, visible) end
end
...
function ui.putback(uobj, go)
	...
	libunity.SetParent(uobj, go, true)
end
...
function ui.setloc(preLang, lang)
	rawset(_G, "lang", lang)
	local TEXT = dofile("localize/_text")--_G.PKG["localize/"..lang]
	OBJDEF.TEXT = TEXT
	rawset(_G, "TEXT", TEXT)
end
